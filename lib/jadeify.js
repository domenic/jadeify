    ...
    return through(
        ...
        function () {
            ...
            readJadeifyConfigFromPackage(process.cwd(), function (err, opts) {
                ...
                getJadeDependencies(inputString, opts.filename, opts).forEach(function (dep) {
                    self.emit("file", dep);
                });
...
function getJadeDependencies(inputString, fileName, opts) {
  var parser = new jade.Parser(inputString, fileName, opts);
  parser.parse();
